...
void digitalWrite (uint32_t *gpio, int pin, int value) {
	...
	int off;
	if (value == LOW) {
		off = (pin > 31) ? 11 : 10; // register number for GPCLR
	}
	else {
		off = (pin > 31) ? 8 : 7; // register number for GPSET
	}
	*(gpio + off) = 1 << (pin & 31);
...
int readButton(uint32_t *gpio, int button) {
	int off;
	off = (button > 31) ? 14 : 13; // register number for GPREAD
	return (*(gpio + off) & (1 << (button & 31)));
  /* ***  COMPLETE the code here, using inline Assembler  ***  */
}
