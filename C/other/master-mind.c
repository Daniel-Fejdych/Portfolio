...
void showMatches(int /* or int* */ code, /* only for debugging */ int* seq1, int* seq2, /* optional, to control layout */ int lcd_format) {
	...
	printf("%d", matches);//maybe change to display on lcd.
...
void readSeq(int* seq, int val) {
	while (val > 0) {
		*seq = val % 10;//use remainder and save int (only works with less then 11 colours (using 0-9 at most))
		seq += 1;
		val /= 10;
	}
	/* ***  the code here  ***  */
...
uint64_t timeInMicroseconds() {
	...
	long long ms = tv.tv_sec * 1000000LL + tv.tv_sec;
	printf("Time in microsecondsL %lld\n", ms);
	return ms;
	/* ***  COMPLETE the code here  ***  */
}
...
void timer_handler(int signum) {
	...
	//jump out of the wait for current color loop!!!
	timed_out = 1;
	/* ***  COMPLETE the code here  ***  */
}
...
/* ********************************************************** */
/* COMPLETED the code for all of the functions in this SECTION */
/* Implement these as C functions in this file                */
/* ********************************************************** */
...
{ // this is just a suggestion of some variable that you may want to use
	...
	printf("Passed point %d", -1);
	if ((fd = open("/dev/mem", O_RDWR | O_SYNC | O_CLOEXEC)) < 0)
		return failure(FALSE, "setup: Unable to open /dev/mem: %s\n", strerror(errno));
	// GPIO:
	gpio = (uint32_t*)mmap(0, BLOCK_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, gpiobase);
	printf("Passed point %d", 0);
	if ((int32_t)gpio == -1)//fix this
  ...
  //	mode which sends the commands as 2 x 4-bit values.
	...
	while (!found) {
		...
		if (debug) {
			waitForEnter();
			attSeq = readNum(colors);
		}
		else {
			for (int i = 0; i < seqlen; i++) {
				//set interval timer for 2 seconds
				initITimer(5);
				timed_out = 0;
				int currCol = 0;
				while (!timed_out) {
		...
		if (countMatches(attSeq, theSeq) / 100 == seqlen) {
			found = 1;/* full match */
		}
		showMatches(0, attSeq, theSeq, 0);
	...
	if (found) {
		/* ***  COMPLETE the code here  ***  */
		fprintf(stdout, "Sequence found in: %d attempts.\n", attempts);
