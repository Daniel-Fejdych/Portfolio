...
def parse_llm_question(text):
    ...
    try:
        ...
        visibility = data.get("VISIBILITY", VISIBILITY_PUBLIC).lower()
        ...
        # --- Case 1: TARGET is a player NAME ---
        for seat, name in SEAT_TO_NAME.items():
            if target_label.lower() == name.lower():
                player_id = seat
                break

        # --- Case 2: fallback for "Player A" format ---
        if player_id is None and target_label.lower().startswith("player"):
            letter = target_label.replace("Player", "").strip().upper()
